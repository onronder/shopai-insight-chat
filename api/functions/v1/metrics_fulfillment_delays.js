...
export default async function handler(req, res) {
  try {
    ...
    // Query the fulfillment_delays view with optional timeframe filter
    let query = supabase
      .from('vw_fulfillment_delays')
      .select('order_id, delay_days')
      .order('delay_days', { ascending: false })
      .limit(20); // Limit to most delayed orders
    ...
    // Return the fulfillment delays data
    res.status(200).json(data || []);
  } catch (error) {
    console.error('API error:', error);
    res.status(500).json({ error: error.message || 'Failed to fetch fulfillment delays data' });
  }
} 