...
serve(async (req) => {
  ...
  try {
    ...
    if (error || !stores) {
      logError("shopify_delta_customers", error, {});
    ...
    return addSecurityHeaders(new Response("Delta customer sync completed", { status: 200 }));
  } catch (err) {
    logError("shopify_delta_customers", err, { path });
