...
serve(async (req) => {
  ...
  try {
    logInfo("metrics_customer_segments", "Request started", { path });
    ...
    if (error) {
      logError("metrics_customer_segments", error, { store_id });
      return addSecurityHeaders(returnJsonError(500, "Error fetching customer segments"));
    }
    ...
    logError("metrics_customer_segments", err, { path });
