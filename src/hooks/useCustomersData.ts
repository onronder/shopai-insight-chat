...
interface CustomerAcquisitionRow {
  customer_id: string;
  period: string;
  total_orders: number;
  store_id: string;
...
export const useCustomersData = () => {
  ...
  const segments = useQuery<CustomerSegment[]>({
    ...
    queryFn: () =>
      fetcher(`/functions/v1/metrics_customer_segments?timeframe=${timeframe}`),
  });

  const ltvDistribution = useQuery<LtvDistribution[]>({
    queryKey: ["ltv_distribution", timeframe],
    queryFn: () =>
      fetcher(`/functions/v1/metrics_ltv_distribution?timeframe=${timeframe}`),
  });

  const churnCandidates = useQuery<ChurnCandidate[]>({
    queryKey: ["churn_candidates", minChurnRisk],
    queryFn: () =>
      fetcher(`/functions/v1/metrics_churn_candidates?min_risk=${minChurnRisk}`),
  });

  const repeatCustomers = useQuery<RepeatCustomer[]>({
    queryKey: ["repeat_customers", timeframe],
    queryFn: () =>
      fetcher(`/functions/v1/metrics_repeat_ratio?timeframe=${timeframe}`),
  });
  ...
  const recent = useQuery<RecentSignup[]>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(
          "customer_id, period, total_orders, store_id, shopify_customers(first_name,last_name,created_at)"
        )
        .eq("period", timeframe)
        .order("created_at", {
          foreignTable: "shopify_customers",
          ascending: false,
        })
        .limit(10);
      ...
      const typedData = data as unknown as CustomerAcquisitionRow[];

      return typedData
        .filter(
          (d) =>
            d.shopify_customers !== null &&
            (d.shopify_customers.first_name || d.shopify_customers.last_name)
        )
        .map((entry) => ({
