      ...
      return (
        <EmptyState
          ...
          action={
            <button
              onClick={() => refetch()}
              className="mt-4 px-4 py-2 text-sm font-medium bg-primary text-white rounded hover:bg-primary/90 transition"
            >
              Refresh Data
            </button>
          }
    ...
    <Card className="col-span-2">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <div>
          ...
          <CardDescription>
            Distribution of customers by their purchasing behavior
          </CardDescription>
        </div>
        <TimeframeSelector
          value={timeframe}
