...
export const BestCustomers: React.FC<BestCustomersProps> = ({ data }) => {
  // Find repeat and new customer data
  const repeatData = data.find(item => item.type === 'repeat') || { count: 0, percentage: 0, type: 'repeat' };
  const newData = data.find(item => item.type === 'new') || { count: 0, percentage: 0, type: 'new' };
      ...
      <CardContent>
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm font-medium">{repeatData.count.toLocaleString()} ({repeatData.percentage}%)</span>
          </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm font-medium">{newData.count.toLocaleString()} ({newData.percentage}%)</span>
          </div>
          <Progress value={newData.percentage} className="h-2" />
          
          <div className="p-3 bg-muted/50 rounded-lg mt-4">
            <p className="text-sm">
              <span className="font-medium">AI Insight:</span> {repeatData.percentage > 50 
